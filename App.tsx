...
import { Grade2ExercisePage } from './pages/grade2/Grade2ExercisePage'; // Added import for 2nd Grade
...
const App: React.FC = () => {
  ...
  const handleMarkExerciseSetCompleted = useCallback((exerciseId: string) => {
    ...
    setCompletedExercises(new Set(updatedSet)); // Create new Set instance for re-render
  }, []);
  ...
  useEffect(() => {
    ...
    setRouteParams(parseHash()); // Initial parse
    setCompletedExercises(getCompletedExercises()); // Initial load of completed exercises
    return () => {
      ...
      if (routeParams.gradeId && GRADES_CONFIG.find(g => g.id === routeParams.gradeId)) { 
        return (
          <GradePage
            ...
            completedExercises={completedExercises} // Pass completed exercises
          />
        );
      }
      navigateToMain(); 
    ...
    case PageView.EXERCISE:
      if (routeParams.gradeId && routeParams.subjectId && routeParams.exerciseId) {
        const pageProps = {
          ...
          onNavigateToAvatar: navigateToAvatar,
          onSetCompleted: handleMarkExerciseSetCompleted, // Pass the completion handler
        };

        if (routeParams.gradeId === 1) {
          return <Grade1ExercisePage {...pageProps} />;
        } else if (routeParams.gradeId === 2) { // Added case for 2nd Grade
