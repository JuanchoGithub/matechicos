...
interface AproximarNumerosMillaresExerciseProps {
...
const FACE_EMOJIS_MILLARES = ['🎯', '🤔', '🧐', '💡', '🤓', '👍', '👌', '💯', '✨', '📏', '🎈', '🏦', '⛰️'];

export const AproximarNumerosMillaresExercise: React.FC<AproximarNumerosMillaresExerciseProps> = ({
  exercise,
  scaffoldApi,
  ...
  const [currentEmoji, setCurrentEmoji] = useState<string>(FACE_EMOJIS_MILLARES[0]);
  const [isAttemptPending, setIsAttemptPending] = useState(false);

  const { minNumber = 1000, maxNumber = 9999 } = exercise.data || {};
  const roundingUnit = 1000;
  ...
  const generateNewChallenge = useCallback(() => {
    ...
    setCurrentEmoji(FACE_EMOJIS_MILLARES[Math.floor(Math.random() * FACE_EMOJIS_MILLARES.length)]);
  ...
  const verifyAnswer = useCallback(() => {
    ...
    if (isCorrect) {
      ...
      showFeedback({ type: 'incorrect', message: 'No es el millar correcto. Revisa las centenas, decenas y unidades y prueba de nuevo.' });
      setIsAttemptPending(false);
    }
    ...
    return (
      <div className="flex flex-col items-center justify-start text-center w-full max-w-lg p-3 space-y-6">
        <div className="relative flex items-center justify-center mb-4">
          ...
          <Icons.SpeechBubbleIcon className="bg-purple-600 text-white text-lg p-3 max-w-xs" direction="left">
            Redondea al millar: <strong className="block text-4xl font-bold">{currentChallenge.numberToRound}</strong>
