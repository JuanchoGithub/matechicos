...
const ExperimentalProbabilitySimulatorExercise: React.FC<{ exercise: Exercise, onAttempt: (success: boolean) => void, setCustomKeypadContent?: (content: React.ReactNode | null) => void }> = ({ 
  ...
  const [activeField, setActiveField] = useState<'numerator' | 'denominator'>('denominator');
  ...
  useEffect(() => {
    if (scenarios && scenarios.length > 0) {
      ...
      // Ensure the denominator field is active by default
      setActiveField('denominator');
      
      // Focus the denominator input once initialized
      setTimeout(() => {
        if (denominatorInputRef.current) {
  ...
  useEffect(() => {
    if (setCustomKeypadContent) {
      ...
      // Set the keypad content immediately
      setCustomKeypadContent(keypadContent);
      
      // Make sure denominator input is focused after keypad is set up
      setTimeout(() => {
        if (denominatorInputRef.current) {
          denominatorInputRef.current.focus();
        }
  ...
  const resetExperiment = () => {
    ...
    setActiveField('denominator'); // Always default to denominator field to ensure keypad works
          ...
          <table className="simulator-results-table">
            ...
            <tbody>
              {outcomes.length > 0 ? (
                ...
                <tr>
                  <td colSpan={3}>Aún no hay resultados</td>
                </tr>
              )}
            </tbody>
          </table>
