...
interface SecuenciaMesAnioExerciseProps {
...
const CHARACTER_EMOJIS_SECUENCIA_MES = ['🗓️', '🤔', '🧐', '⏪', '⏩', '💡', '📅'];
const ALL_MONTHS_SEQ = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];

export const SecuenciaMesAnioExercise: React.FC<SecuenciaMesAnioExerciseProps> = ({
  ...
  const [characterEmoji, setCharacterEmoji] = useState<string>(CHARACTER_EMOJIS_SECUENCIA_MES[0]);
  
  const { months: monthPool = ALL_MONTHS_SEQ } = exercise.data || { months: ALL_MONTHS_SEQ };
  ...
  const loadNewChallenge = useCallback(() => {
    ...
    if (currentPool.length > 0) {
      ...
      const currentIndex = ALL_MONTHS_SEQ.indexOf(monthName);
      setCorrectAnterior(currentIndex > 0 ? ALL_MONTHS_SEQ[currentIndex - 1] : "N/A"); 
      setCorrectPosterior(currentIndex < ALL_MONTHS_SEQ.length - 1 ? ALL_MONTHS_SEQ[currentIndex + 1] : "N/A"); 
      setAvailableMonths(prev => prev.slice(1));
      setCharacterEmoji(CHARACTER_EMOJIS_SECUENCIA_MES[Math.floor(Math.random() * CHARACTER_EMOJIS_SECUENCIA_MES.length)]);
    } else {
      onAttempt(true); 
      return;
    }
  ...
  const MainContent: React.FC = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-start text-center w-full max-w-xl p-3 space-y-4">
        ...
        <div className="text-xl sm:text-2xl font-semibold text-slate-700">El mes actual es: <strong className="text-blue-600">{currentMonth.toUpperCase()}</strong></div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-md mt-4">
          <div className="flex flex-col items-center">
            <label htmlFor="mes-ant" className="text-lg font-medium text-slate-600 mb-1">Mes Anterior:</label>
            <select id="mes-ant" value={selectedAnterior} onChange={(e) => setSelectedAnterior(e.target.value)} disabled={(isVerified && selectedAnterior === correctAnterior) || correctAnterior === "N/A"} className="p-2 border border-slate-300 rounded-md shadow-sm text-md w-full focus:ring-sky-500 focus:border-sky-500">
              <option value="">{correctAnterior === "N/A" ? "(Ninguno)" : "Selecciona..."}</option>
              {ALL_MONTHS_SEQ.map(m => <option key={`ant-${m}`} value={m}>{m}</option>)}
            </select>
          </div>
          <div className="flex flex-col items-center">
            <label htmlFor="mes-pos" className="text-lg font-medium text-slate-600 mb-1">Mes Posterior:</label>
            <select id="mes-pos" value={selectedPosterior} onChange={(e) => setSelectedPosterior(e.target.value)} disabled={(isVerified && selectedPosterior === correctPosterior) || correctPosterior === "N/A"} className="p-2 border border-slate-300 rounded-md shadow-sm text-md w-full focus:ring-sky-500 focus:border-sky-500">
              <option value="">{correctPosterior === "N/A" ? "(Ninguno)" : "Selecciona..."}</option>
              {ALL_MONTHS_SEQ.map(m => <option key={`pos-${m}`} value={m}>{m}</option>)}
            </select>
          </div>
        </div>
      </div>
