...
const ProbabilityWordProblemExercise: React.FC<ProbabilityWordProblemExerciseProps> = ({
  ...
  // Handle keypad key press - with useCallback for stability
  ...
  }, [activeField, denominator.length, isAnimating, numerator.length, showFeedback]);

  // Handle backspace button press - with useCallback for stability
  ...
  // Switch between numerator and denominator fields - with useCallback for stability
  const handleFieldClick = useCallback((field: 'numerator' | 'denominator') => {
    if (isAnimating || showFeedback) return;
    setActiveField(field);
  }, [isAnimating, showFeedback]);

  // Check the user's answer - with useCallback for stability
  ...
  }, [denominator, isAnimating, numerator, safeApi, scenario]);

  // Handle item selection
  const handleItemSelect = useCallback((itemLabel: string) => {
    if (isAnimating || showFeedback) return;
  ...
  const handleNextScenario = useCallback(() => {
    // Always reset the states regardless of whether we're changing scenarios or not
    setNumerator('');
    setDenominator('');
    setFeedbackMessage('');
    setShowFeedback(false);
  ...
  }, [handleBackspace, handleCheckAnswer, handleKeyPress, showFeedback]);
  ...
  // Create a ref to track previous signal value to prevent loops
  const prevSignalRef = useRef(safeApi.advanceToNextChallengeSignal);
  
  // Pick a new scenario when the challenge advances, but only when the signal actually changes
  useEffect(() => {
    // Only proceed if the signal has actually changed and is greater than 0
    if (safeApi.advanceToNextChallengeSignal > 0 && 
        safeApi.advanceToNextChallengeSignal !== prevSignalRef.current && 
        scenarios.length > 1) {
      let newIndex;
      do {
        newIndex = Math.floor(Math.random() * scenarios.length);
      ...
      // Update the ref to the current signal value
      prevSignalRef.current = safeApi.advanceToNextChallengeSignal;
    }
  }, [safeApi.advanceToNextChallengeSignal, scenarios.length]);
