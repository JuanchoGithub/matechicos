...
interface CurrentProblem {
  ...
  divisor?: 2 | 3 | 5 | 10 | 100 | 1000; // Updated type
}
...
export const CalculaMentalmenteExercise: React.FC<CalculaMentalmenteExerciseProps> = ({
  ...
  const generateNewProblem = useCallback(() => {
    ...
    let divisorVal: 2 | 3 | 5 | 10 | 100 | 1000 | undefined; // Added 100, 1000 for divideByPowerOfTen
    ...
    switch (effectiveProblemType) {
      ...
      case 'multiplyByPowerOfTen':
        num1 = Math.floor(Math.random() * (maxOperand - minOperand + 1)) + minOperand;
        const powersMult: (10 | 100 | 1000)[] = [10, 100, 1000];
        powerOfTenVal = powersMult[Math.floor(Math.random() * powersMult.length)];
        ...
        break;
      case 'divideByPowerOfTen':
        const powersDiv: (10 | 100 | 1000)[] = [10, 100, 1000];
        divisorVal = powersDiv[Math.floor(Math.random() * powersDiv.length)];
        const quotientForPowerDiv = Math.floor(Math.random() * (Math.floor(maxOperand / divisorVal) - Math.ceil(minOperand / divisorVal) + 1)) + Math.ceil(minOperand / divisorVal);
        if (quotientForPowerDiv === 0 && minOperand > 0) { // Avoid 0 / X if minOperand is > 0
             num1 = divisorVal; // e.g. 10 / 10 = 1
             answer = 1;
        } else {
            num1 = quotientForPowerDiv * divisorVal;
            answer = quotientForPowerDiv;
        }
        if (num1 < minOperand || num1 > maxOperand) { // Safety check if ranges are tricky
            num1 = (Math.floor(Math.random() * 9) +1) * divisorVal * 10; // Generate a clear multiple within a broader range
            if (num1 > maxOperand) num1 = divisorVal * 10;
            if (num1 < minOperand) num1 = minOperand - (minOperand % divisorVal) + divisorVal; // ensure it's a multiple
            answer = num1 / divisorVal;
        }

        problemStr = `${num1} ÷ ${divisorVal}`;
        setCurrentProblem({ problemString: problemStr, correctAnswer: answer, operand1: num1, divisor: divisorVal as 10|100|1000 });
        break;
      case 'divideByBasicDivisor':
        const divisorsBasic: (2 | 3 | 5 | 10)[] = [2, 3, 5, 10];
        divisorVal = divisorsBasic[Math.floor(Math.random() * divisorsBasic.length)];
        const quotientBasic = Math.floor(Math.random() * (maxResultOperand - minResultOperand + 1)) + minResultOperand;
        num1 = quotientBasic * divisorVal; 
        answer = quotientBasic; 
        problemStr = `${num1} ÷ ${divisorVal}`;
        setCurrentProblem({ problemString: problemStr, correctAnswer: answer, operand1: num1, divisor: divisorVal as 2|3|5|10 });
        break;
  ...
  const MainContent: React.FC = () => {
    ...
    } else if (problemType === 'multiplyByPowerOfTen' || problemType === 'divideByPowerOfTen' || problemType === 'divideByBasicDivisor'){
        currentSpeechBubbleIcon = <Icons.OperationsIcon className="w-8 h-8 inline-block" />;
