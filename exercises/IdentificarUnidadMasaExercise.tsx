...
interface IdentificarUnidadMasaExerciseProps {
...
const FACE_EMOJIS = ['⚖️', '🤔', '🧐', '💡', '🤓', '👍', '🎯', '💯', '🍎', '🍉', '🧱', '🪶'];
const UNITS_OPTIONS: { unit: MassUnit; label: string; fullLabel: string }[] = [
  { unit: 'g', label: 'g', fullLabel: 'Gramo' },
  { unit: 'kg', label: 'kg', fullLabel: 'Kilogramo' },
];
...
export const IdentificarUnidadMasaExercise: React.FC<IdentificarUnidadMasaExerciseProps> = ({
  exercise,
  scaffoldApi,
}) => {
  // stars and feedback states removed
  const [currentChallenge, setCurrentChallenge] = useState<CurrentChallengeData | null>(null);
  const [selectedUnit, setSelectedUnit] = useState<MassUnit | null>(null);
  ...
  const generateNewChallenge = useCallback(() => {
    ...
    if (itemPool.length > 0) {
        ...
        scaffoldApi.onAttempt(true); // Signal completion if no more challenges
        return;
    }
  ...
  const handleUnitSelect = (unit: MassUnit) => {
  ...
  const verifyAnswer = useCallback(() => {
    ...
    if (isCorrect) {
      scaffoldApi.showFeedback({ type: 'correct', message: `¡Correcto! Usarías ${selectedUnit} para pesar ${currentChallenge.description}.` });
      ...
      <div className="flex flex-col items-center justify-start text-center w-full max-w-xl p-3 space-y-4 sm:space-y-6">
        <div className="relative flex items-center justify-center mb-2 sm:mb-4">
          ...
          <Icons.SpeechBubbleIcon className="bg-green-600 text-white text-md sm:text-lg p-2 sm:p-3 max-w-xs" direction="left">
             Pesar: <strong className="block text-lg sm:text-xl">{currentChallenge.description}</strong>
