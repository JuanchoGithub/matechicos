      ...
      {options.map((opt, index) => {
        ...
        return (
          <button
            ...
            disabled={isVerified && selectedOption?.isCorrect}
...
export const ExpresarProbabilidadSimpleExercise: React.FC<ExpresarProbabilidadSimpleExerciseProps> = ({
  ...
  const [availableScenarios, setAvailableScenarios] = useState<SimpleProbabilityScenario[]>([]);
  ...
  useEffect(() => {
    if (scenarios.length > 0) {
      setAvailableScenarios(shuffleArray([...scenarios as SimpleProbabilityScenario[]]));
    }
  }, [scenarios]);

  const loadNewChallenge = useCallback(() => {
    let scenarioPool = availableScenarios;
    if (scenarioPool.length === 0 && (scenarios as SimpleProbabilityScenario[]).length > 0) {
      scenarioPool = shuffleArray([...scenarios as SimpleProbabilityScenario[]]);
    }

    if (scenarioPool.length > 0) {
      const nextScenario = {...scenarioPool[0]}; 
      nextScenario.options = shuffleArray([...nextScenario.options]); 
      setCurrentChallenge(nextScenario);
      setAvailableScenarios(prev => prev.slice(1));
      setCurrentEmoji(nextScenario.emoji || FACE_EMOJIS_EXPRESAR[Math.floor(Math.random() * FACE_EMOJIS_EXPRESAR.length)]);
    } else {
      onAttempt(true);
      return;
    }
  ...
  const handleOptionSelect = useCallback((option: ProbabilityOption) => {
    if (isVerified && selectedOption?.isCorrect) return;
    setSelectedOption(option);
    showFeedback(null);
    if (isVerified && !selectedOption?.isCorrect) setIsVerified(false);
  ...
  const verifyAnswer = useCallback(() => {
    ...
    onAttempt(isCorrect);
    if (isCorrect) {
      showFeedback({ type: 'correct', message: `¡Correcto! La probabilidad es ${selectedOption.text}.` });
    } else {
      const correctAnswerText = currentChallenge.options.find(opt => opt.isCorrect)?.text || "la opción correcta";
      showFeedback({ type: 'incorrect', message: `No es correcto. La probabilidad era ${correctAnswerText}.` });
    }
  ...
  const MainContent: React.FC = () => {
    if (!currentChallenge) {
      return <div className="p-4 text-xl text-slate-600">Cargando escenario...</div>;
    }
    const { contextText, visualContext, eventToEvaluate } = currentChallenge;
    return (
      <div className="flex flex-col items-center justify-start text-center w-full max-w-2xl p-3 space-y-3">
        <div className="relative flex items-center justify-center mb-2">
          ...
          <Icons.SpeechBubbleIcon className="bg-teal-600 text-white text-md p-2 max-w-[280px]" direction="left">
            {contextText}
          </Icons.SpeechBubbleIcon>
        </div>
        <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg shadow-sm w-full">
          <p className="text-lg font-bold text-yellow-700 mb-1">{visualContext}</p>
          <p className="text-md text-slate-700">¿Cuál es la probabilidad de: <strong className="text-yellow-800">{eventToEvaluate}</strong>?</p>
        </div>
      </div>
    );
  };
