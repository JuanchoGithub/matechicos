...
interface SumarNumeros2CifrasExerciseProps {
...
export const SumarNumeros2CifrasExercise: React.FC<SumarNumeros2CifrasExerciseProps> = ({
  exercise,
  currentAvatar,
  onNavigateBack,
  onGoHome,
  ...
  const generateNewProblem = useCallback(() => {
    const num1 = Math.floor(Math.random() * (maxOperand - minOperand + 1)) + minOperand;
    const num2 = Math.floor(Math.random() * (maxOperand - minOperand + 1)) + minOperand;
    const answer = num1 + num2;
    const problemStr = `${num1} + ${num2}`;
  ...
  const verifyAnswer = useCallback(() => {
    ...
    if (userAnswer === currentProblem.correctAnswer) {
      ...
      setFeedback({ type: 'correct', message: '¡Muy bien! Suma correcta.' });
      onComplete(true);
      ...
      setFeedback({ type: 'incorrect', message: 'Respuesta incorrecta. Revisa tu suma e inténtalo de nuevo.' });
      onComplete(false);
    }
  ...
  return (
    <ExerciseScaffold
      ...
      exerciseQuestion={exercise.question || `Resuelve la suma:`}
