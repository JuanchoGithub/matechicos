...
// Estadistica G5
import { LinePlotCreatorExercise } from '../../exercises/LinePlotCreatorExercise';
import { BarGraphBuilderExercise } from '../../exercises/BarGraphBuilderExercise';
import { MeanValueMissionExercise } from '../../exercises/MeanValueMissionExercise';
import { RangeFinderChallengeExercise } from '../../exercises/RangeFinderChallengeExercise';
...
import { CoordinatePlaneNavigatorG5Exercise } from '../../exercises/grade5/CoordinatePlaneNavigatorG5Exercise';

// Probabilidad G5
import { LikelihoodSortingExercise } from '../../exercises/LikelihoodSortingExercise';
import SpinnerProbabilityExercise from '../../exercises/SpinnerProbabilityExercise';
import CoinDiceProbabilityExercise from '../../exercises/CoinDiceProbabilityExercise';
import ProbabilityWordProblemExercise from '../../exercises/ProbabilityWordProblemExercise';
import ExperimentalProbabilitySimulatorExercise from '../../exercises/ExperimentalProbabilitySimulatorExercise';
...
import DataCollectionQuestExercise from '../../exercises/DataCollectionQuestExercise';
...
export const Grade5ExercisePage: React.FC<Grade5ExercisePageProps> = ({ gradeId, subjectId, exerciseId, onNavigateBack, onNavigateToMain, onNavigateToAvatar, onSetCompleted }) => {
  ...
  // Initialize keypad for certain exercises that need it from the start
  useEffect(() => {
    if (exerciseData?.componentType === ExerciseComponentType.SPINNER_PROBABILITY || 
        exerciseData?.componentType === ExerciseComponentType.COIN_DICE_PROBABILITY || 
        exerciseData?.componentType === ExerciseComponentType.PROBABILITY_WORD_PROBLEM ||
        exerciseData?.componentType === ExerciseComponentType.EXPERIMENTAL_PROBABILITY_SIMULATOR) {
      // Pre-initialize keypad for probability exercises to avoid "no keypad" message
      setCustomKeypadContentG5(
        <div className="sidebar-keypad-container">
          <h3 className="sidebar-keypad-title">
            Numerador
          </h3>
          <NumericKeypad onKeyPress={(value) => {
            // This will be replaced when the actual exercise mounts, just a placeholder
            console.log("Initial keypad pressed:", value);
          }} className="w-full" allowDecimal={false} />
          <div className="sidebar-keypad-buttons">
            <button className="carnival-button check-button" onClick={() => {
              console.log("Initial verify button clicked");
            }}>
              Verificar
            </button>
          </div>
  ...
  const createExerciseContentProps = (api: ExerciseScaffoldApi) => ({
    ...
    gradeLevel: gradeId,
    onAttempt: (success: boolean) => {
      api.onAttempt(success);
      if (success) {
        onSetCompleted(exerciseData.id);
      }
    }
  });
  ...
  switch (exerciseData.componentType) {
    ...
    /* Commented out due to missing components - not related to our task
    ...
    */
    ...
    case ExerciseComponentType.COORDINATE_PLANE_NAVIGATOR_G5: mainContentRenderer = (api) => <CoordinatePlaneNavigatorG5Exercise {...createExerciseContentProps(api)} />; break;
    
    // --- Estadistica ---
    case ExerciseComponentType.LINE_PLOT_CREATOR_G5: mainContentRenderer = (api) => <LinePlotCreatorExercise {...createExerciseContentProps(api)} />; break;
    case ExerciseComponentType.BAR_GRAPH_BUILDER: mainContentRenderer = (api) => <BarGraphBuilderExercise {...createExerciseContentProps(api)} />; break;
    case ExerciseComponentType.DATA_COLLECTION_QUEST: mainContentRenderer = (api) => <DataCollectionQuestExercise {...createExerciseContentProps(api)} />; break;
    case ExerciseComponentType.MEAN_VALUE_MISSION: mainContentRenderer = (api) => <MeanValueMissionExercise {...createExerciseContentProps(api)} />; break;
    case ExerciseComponentType.RANGE_FINDER_CHALLENGE: mainContentRenderer = (api) => <RangeFinderChallengeExercise {...createExerciseContentProps(api)} />; break;
    
    // --- Probabilidad ---
    case ExerciseComponentType.LIKELIHOOD_SORTING_GAME:
      mainContentRenderer = (api) => <LikelihoodSortingExercise {...createExerciseContentProps(api)} />;
      break;
    case ExerciseComponentType.SPINNER_PROBABILITY: 
      mainContentRenderer = (api) => <SpinnerProbabilityExercise {...createExerciseContentProps(api)} />; 
      break;
    case ExerciseComponentType.COIN_DICE_PROBABILITY: 
      mainContentRenderer = (api) => <CoinDiceProbabilityExercise {...createExerciseContentProps(api)} />; 
      break;
    case ExerciseComponentType.PROBABILITY_WORD_PROBLEM: 
      mainContentRenderer = (api) => <ProbabilityWordProblemExercise {...createExerciseContentProps(api)} />; 
      break;
    case ExerciseComponentType.EXPERIMENTAL_PROBABILITY_SIMULATOR: 
      mainContentRenderer = (api) => <ExperimentalProbabilitySimulatorExercise {...createExerciseContentProps(api)} />; 
      break;

    // --- G5 Problemas ---
    case ExerciseComponentType.PROBLEMAS_MULTIPLES_PASOS_G5:
      mainContentRenderer = (api) => <ProblemasMultiplesPasosG5Exercise {...createExerciseContentProps(api)} />;
      break;
    case ExerciseComponentType.PROBLEMAS_PROPORCIONALIDAD_G5:
      mainContentRenderer = (api) => <ProporcionalidadG5Exercise {...createExerciseContentProps(api)} />;
      break;
    case ExerciseComponentType.PROBLEMAS_INTERPRETAR_RESTO_G5:
      mainContentRenderer = (api) => <InterpretarRestoG5Exercise {...createExerciseContentProps(api)} />;
      break;
    case ExerciseComponentType.FINANZAS_AVANZADO:
      mainContentRenderer = (api) => <FinanzasAvanzadoExercise {...createExerciseContentProps(api)} />;
      break;
