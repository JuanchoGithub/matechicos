...
const DEFAULT_EXERCISE_STARS_G2 = 5; 
...
export const Grade2ExercisePage: React.FC<Grade2ExercisePageProps> = ({ gradeId, subjectId, exerciseId, onNavigateBack, onNavigateToMain, onNavigateToAvatar, onSetCompleted }) => {
  ...
  const [exerciseSpecificKeypadHandlerG2, setExerciseSpecificKeypadHandlerG2] = useState<KeypadHandler | null>(null);
  ...
  const handleApiReady = useCallback((api: ExerciseScaffoldApi) => {
    ...
    if (newSignalValue !== undefined && newSignalValue !== previousSignalValueFromScaffoldRef.current) {
      ...
      // Add other G2 specific challenge regeneration calls here if they are managed by ExercisePage state
  ...
  useEffect(() => {
    ...
    setExerciseSpecificKeypadHandlerG2(null);
    ...
    // ... else clear states for other managed exercises
    else { setComparisonChallengeG2(null); setCurrentTimeChallengeRelojG2(null); }
  }, [exerciseId, subjectId, gradeId, exerciseData, generateNewComparisonChallengeG2, generateNewTimeChallengeRelojG2]);
  
  const registerKeypadHandlerG2 = useCallback((handler: KeypadHandler | null) => {
    setExerciseSpecificKeypadHandlerG2(() => handler);
  }, []);

  const setSidebarContentForExerciseG2 = useCallback((keypadNode: React.ReactNode | null) => { 
  ...
  const createExerciseContentProps = (api: ExerciseScaffoldApi) => ({
    ...
    setCustomKeypadContent: setSidebarContentForExerciseG2, 
    gradeLevel: gradeId, 
  });
  
  let mainContentRenderer: (api: ExerciseScaffoldApi) => React.ReactNode;
  let keypadToRenderG2: React.ReactNode = null; 
  let allowDecimalForKeypadG2 = false;
  ...
  const isSidebarExercise = 
    ...
    exerciseData.componentType === ExerciseComponentType.BALANZA_1KG || // G1 reuse
    exerciseData.componentType === ExerciseComponentType.SECUENCIA_DIA_SEMANA || // G1 reuse
    exerciseData.componentType === ExerciseComponentType.SECUENCIA_MES_ANIO || // G1 reuse
    ...
    exerciseData.componentType === ExerciseComponentType.SIMPLE_CHANCE_GAME ||
  ...
  switch (exerciseData.componentType) {
    ...
    case ExerciseComponentType.IDENTIFICAR_ORDINALES: mainContentRenderer = (api) => <IdentificarOrdinalesExercise {...createExerciseContentProps(api)} setCustomKeypadContent={setSidebarContentForExerciseG2} />; break; 
    case ExerciseComponentType.IDENTIFICAR_PARES_IMPARES: mainContentRenderer = (api) => <IdentificarParesImparesExercise {...createExerciseContentProps(api)} setCustomKeypadContent={setSidebarContentForExerciseG2} />; break;
    ...
    case ExerciseComponentType.REPRESENTAR_FRACCIONES: mainContentRenderer = (api) => <RepresentarFraccionesExercise {...createExerciseContentProps(api)} setCustomKeypadContent={setSidebarContentForExerciseG2} />; break;
    ...
    // Problemas
    case ExerciseComponentType.PROBLEMAS_PASO_A_PASO: mainContentRenderer = (api) => <ProblemasMatematicosGuiadosExercise {...createExerciseContentProps(api)} setCustomKeypadContent={setSidebarContentForExerciseG2} />; break; 
    // Medidas
    ...
    case ExerciseComponentType.BALANZA_1KG: mainContentRenderer = (api) => <Balanza1KgExercise {...createExerciseContentProps(api)} setCustomKeypadContent={setSidebarContentForExerciseG2} />; break;
    case ExerciseComponentType.LEER_RELOJ_OPCIONES: mainContentRenderer = () => (<LeerRelojOpcionesExercise hours={currentTimeChallengeRelojG2?.hours ?? 0} minutes={currentTimeChallengeRelojG2?.minutes ?? 0} currentEmoji={relojEmojiG2} questionText={exerciseData.question || "¿Qué hora es?"}/>); keypadToRenderG2 = (<TimeOptionsKeypad options={currentTimeChallengeRelojG2?.options || []} selectedOption={selectedTimeOptionRelojG2} onOptionSelect={handleTimeOptionSelectRelojG2} onVerify={handleVerifyTimeAnswerRelojG2} isVerified={isVerifiedRelojG2} correctAnswerText={currentTimeChallengeRelojG2?.correctText || null}/>); break;
    case ExerciseComponentType.SECUENCIA_DIA_SEMANA: mainContentRenderer = (api) => <SecuenciaDiaSemanaExercise {...createExerciseContentProps(api)} setCustomKeypadContent={setSidebarContentForExerciseG2} />; break;
    case ExerciseComponentType.SECUENCIA_MES_ANIO: mainContentRenderer = (api) => <SecuenciaMesAnioExercise {...createExerciseContentProps(api)} setCustomKeypadContent={setSidebarContentForExerciseG2} />; break;
    ...
    // Geometria
    case ExerciseComponentType.IDENTIFICAR_FORMA_GEOMETRICA_GENERICO: mainContentRenderer = (api) => <IdentificarFormaGeometricaGenericoExercise {...createExerciseContentProps(api)} setExternalKeypad={setSidebarContentForExerciseG2} />; break;
    // Estadistica
    case ExerciseComponentType.INTERPRETAR_PICTOGRAMAS: mainContentRenderer = (api) => <InterpretarPictogramasExercise {...createExerciseContentProps(api)} registerKeypadHandler={registerKeypadHandlerG2} />; break; // It sets its own keypad or uses numeric based on question type.
    case ExerciseComponentType.INTERPRETAR_TABLA_SIMPLE: mainContentRenderer = (api) => <InterpretarTablaSimpleExercise {...createExerciseContentProps(api)} setExternalKeypad={setSidebarContentForExerciseG2} />; break; 
    // Probabilidad
    case ExerciseComponentType.CERTEZA_SUCESOS: mainContentRenderer = (api) => <CertezaSucesosExercise {...createExerciseContentProps(api)} setCustomKeypadContent={setSidebarContentForExerciseG2} />; break;
    case ExerciseComponentType.SIMPLE_CHANCE_GAME: mainContentRenderer = (api) => <SimpleChanceGameExercise {...createExerciseContentProps(api)} setCustomKeypadContent={setSidebarContentForExerciseG2} />; break;
    
    case ExerciseComponentType.GENERIC: default: mainContentRenderer = (api) => <GenericExerciseDisplay exerciseData={exerciseData} gradeThemeColor={gradeThemeColor} onNavigateBack={onNavigateBack} onNavigateToAvatar={onNavigateToAvatar} currentAvatar={currentAvatar} />; keypadToRenderG2 = null; break;
  }
  
  let finalKeypadComponentForScaffold: React.ReactNode = customKeypadContentG2; 
  if (!finalKeypadComponentForScaffold) { 
    if (keypadToRenderG2) { 
        finalKeypadComponentForScaffold = keypadToRenderG2;
    } else if (exerciseSpecificKeypadHandlerG2) { 
        finalKeypadComponentForScaffold = <NumericKeypad onKeyPress={exerciseSpecificKeypadHandlerG2} className="w-full" allowDecimal={allowDecimalForKeypadG2} />;
    } else {
